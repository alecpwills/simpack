...
def wham2d_command(rootdir, metafile, histminx=2, histmaxx=6, numbinsx=41,
                 histminy=2, histmaxy=2, numbinsy=41, temp=300, tol=1e-8, output='PMF', returnarr=True):
    cwd = os.getcwd()
    os.chdir(rootdir)
    inp = os.path.join(rootdir, metafile)
    out = os.path.join(rootdir, output)
    execstr = "wham2d Px=0 {} {} {} Py=0 {} {} {} {} {} 0 {} {} 0".format(histminx, histmaxx, numbinsx,
                                                   histminy, histmaxy, numbinsy,
                                                   tol, temp, inp, out)
    print(execstr)
    os.chdir(cwd)
    try:
